...
struct cmoe_vector{
  ...
  cmoe_vector(SEXP x){
    SEXP dimsS = PROTECT(Rf_getAttrib(x,install("vdim")));
...
struct cmoe_matrix{
  ...
  cmoe_matrix(SEXP x){
    SEXP dimcS = PROTECT(Rf_getAttrib(x,install("cdim")));
    SEXP dimrS = PROTECT(Rf_getAttrib(x,install("rdim")));
...
struct cmoe_3darray{
  ...
  cmoe_3darray(SEXP x){
    SEXP dimcS = PROTECT(Rf_getAttrib(x,install("cdim")));
    SEXP dimrS = PROTECT(Rf_getAttrib(x,install("rdim")));
    SEXP dimaS = PROTECT(Rf_getAttrib(x,install("adim")));
    int nv = Rf_length(dimcS);
    ncol_ = nv;
 ...
 // Addition with scalars
  ...
  friend cmoe_3darray<Type> operator+(cmoe_3darray<Type> lhs, const Type& rhs){
    lhs += rhs;
    return lhs;
  }
  // Subtraction with scalars
...
template<class Type>
cmoe_3darray<Type> asCmoe3DArray(SEXP x){
  return cmoe_3darray<Type>(x);
}

// template<class Type>
// SEXP asSEXP(const cmoe_3darray<Type>& x){
//   return asSEXP(x.dat);
// }
...
#define PARAMETER_CMOE_VECTOR(name) cmoe_vector<Type> name(objective_function::fillShape(asCmoeVector<Type>(objective_function::getShape(#name,&Rf_isArray)),#name));

#define PARAMETER_CMOE_MATRIX(name) cmoe_matrix<Type> name(objective_function::fillShape(asCmoeMatrix<Type>(objective_function::getShape(#name,&Rf_isArray)),#name));


#define PARAMETER_CMOE_3DARRAY(name) cmoe_3darray<Type> name(objective_function::fillShape(asCmoe3DArray<Type>(objective_function::getShape(#name,&Rf_isArray)),#name));
